...
// ⚡ Bolt Optimization:
// Wrap ModelCard in React.memo() to prevent O(N) re-renders when parent components
// (like Explore or Home) re-render due to search input keystrokes or unrelated state changes.
// Expected Impact: Eliminates unnecessary re-renders of the entire model list, making typing in search boxes significantly smoother.
const ModelCard = React.memo(({ model, compact = false }) => {
  const thumbnail = getThumbnail(model);
  const format =
    getModelFileFormat(model) || model?.fileFormat || model?.format || '';
  const category = model?.category || '3D Model';
  const tags = Array.isArray(model?.tags)
    ? model.tags.slice(0, compact ? 1 : 2)
    : [];
  ...
  return (
    <Link to={getModelUrl(model)} className="asset-card group">
      ...
      <div className="asset-card__body">
        ...
        <h3 className="asset-card__title line-clamp-2">
        ...
        <div className="asset-card__meta">
          <span>
            {formatCount(model?.downloads || model?.downloads_count)} downloads
          </span>
        ...
        {tags.length > 0 && (
          <div className="asset-card__tags">
            {tags.map(tag => (
              <span key={tag} className="asset-card__tag">
                {tag}
              </span>
            ))}
          </div>
...
// Optional: Add a display name for React DevTools debugging since it's an anonymous function inside memo
