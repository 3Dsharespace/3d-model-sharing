...
const getModelStatus = (model) => {
  ...
  if (model.isDraft === true || model.is_private === true || model.isPublic === false || model.is_public === false) return 'draft'
  return 'published'
}

const isModelVisible = (model) => (
  model.is_private !== true &&
  model.isPublic !== false &&
  model.is_public !== false &&
  getModelStatus(model) !== 'draft' &&
  getModelStatus(model) !== 'archived'
)

const getVisibilityUpdate = (status, isPublic = status !== 'draft' && status !== 'archived') => ({
  status,
...
const ModelManagementTab = () => {
  ...
  const filteredModels = models.filter(model => {
    ...
    const matchesStatus = selectedStatus === 'all' || modelStatus === selectedStatus
    ...
    const matchesDraft = filters.isDraft === 'all' ||
      (filters.isDraft === 'true' && modelStatus === 'draft') ||
      (filters.isDraft === 'false' && modelStatus !== 'draft')
  ...
  const performModelAction = async (modelId, action) => {
    ...
    try {
      switch (action) {
        case 'publish':
          await firebaseHelpers.updateModel(modelId, getVisibilityUpdate('published', true))
          break
        case 'draft':
          await firebaseHelpers.updateModel(modelId, getVisibilityUpdate('draft', false))
          break
        case 'feature':
          await firebaseHelpers.updateModel(modelId, { 
            ...getVisibilityUpdate('featured', true),
            status: 'featured',
            isFeatured: true
          })
          break
        case 'unfeature':
          await firebaseHelpers.updateModel(modelId, { 
            ...getVisibilityUpdate('published', true),
            status: 'published',
            isFeatured: false
          })
          break
        case 'archive':
          await firebaseHelpers.updateModel(modelId, getVisibilityUpdate('archived', false))
          break
        case 'delete':
    ...
    >
      <div className="relative">
        ...
        <div className="absolute top-2 left-2 flex flex-col space-y-1">
          <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(getModelStatus(model))}`}>
    ...
    >
      ...
      <td className="px-6 py-4">
        <div className="flex flex-col space-y-1">
          <span className={`px-2 py-1 text-xs rounded-full w-fit ${getStatusColor(getModelStatus(model))}`}>
            {getModelStatus(model)}
          </span>
          {model.isPlatformModel && (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(getModelStatus(selectedModel))}`}>
...
const EditModelForm = ({ model, onSave, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedData = {
        ...formData,
        ...getVisibilityUpdate(formData.status, formData.isPublic && formData.status !== 'draft' && formData.status !== 'archived'),
