...
import { getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut, sendPasswordResetEmail, GoogleAuthProvider, signInWithPopup, signInWithRedirect, getRedirectResult, setPersistence, browserLocalPersistence } from "firebase/auth";
...
const writeUserProfileInBackground = (user, data = {}) => {
  if (!user?.uid) return;

  const fallbackName = user.displayName || user.email?.split('@')[0] || data.username || 'creator';
  const profile = {
    uid: user.uid,
    ...
    isVerified: Boolean(user.emailVerified || data.isVerified),
    ...data,
    uid: user.uid,
    email: user.email || data.email || '',
    createdAt: user.metadata?.creationTime || data.createdAt || new Date().toISOString()
  };

  setDoc(doc(db, 'users', user.uid), profile, { merge: true }).catch((error) => {
    console.warn('Background profile write failed:', error);
  });
};
...
export const firebaseHelpers = {
  ...
  signUp: async (email, password, username) => {
    try {
      ...
      writeUserProfileInBackground(user, {
        username,
  ...
  resetPassword: async (email) => {
    try {
      await sendPasswordResetEmail(auth, String(email || '').trim(), {
        url: 'https://3dsharespace.com/login',
      ...
      const messages = {
        ...
        'auth/too-many-requests': 'Too many reset attempts. Please wait a few minutes and try again.',
  ...
  checkAdminStatus: async (uid) => {
    try {
      ...
      if (docSnap.exists()) {
        ...
        return { success: true, isAdmin: hasAdminRole(profile) };
      } else {
  ...
  getModels: async (filters = {}) => {
    try {
      ...
      if (filters.userId) {
        ...
        // Filter models by the actual UID. Older uploads may use legacy owner fields.
        models = models.filter(model => (
          model.userId === actualUserId ||
          model.user_id === actualUserId ||
          model.creatorId === actualUserId ||
          model.authorId === actualUserId ||
          model.ownerId === actualUserId
        ));
  ...
  createRazorpayOrder: async ({ amount, currency = 'INR', creatorId, message, type = 'tip', testMode = false, modelId = null, modelTitle = null }) => {
    try {
      const { httpsCallable } = await import('firebase/functions');
      const functions = await getFirebaseService('functions');
      const fn = httpsCallable(functions, 'createRazorpayOrder');
      const res = await fn({ amount, currency, creatorId, message, type, testMode, modelId, modelTitle });
  ...
  verifyRazorpayPayment: async ({ paymentId, orderId, signature, creatorId, amount, message = '', testMode = false, modelId = null, modelTitle = null }) => {
    try {
      const { httpsCallable } = await import('firebase/functions');
      const functions = await getFirebaseService('functions');
      const fn = httpsCallable(functions, 'verifyRazorpayPayment');
      const res = await fn({ paymentId, orderId, signature, creatorId, amount, message, testMode, modelId, modelTitle });
      return { success: true, ...res.data };
    } catch (error) {
  ...
  getConversationId: (firstUserId, secondUserId) => {
    ...
    try {
      ...
      return onSnapshot(conversationsQuery, (snapshot) => {
        const conversations = snapshot.docs
          ...
          .sort((a, b) => {
            const aTime = a.updatedAt?.toDate?.()?.getTime?.() || a.lastMessageAt?.toDate?.()?.getTime?.() || 0;
            const bTime = b.updatedAt?.toDate?.()?.getTime?.() || b.lastMessageAt?.toDate?.()?.getTime?.() || 0;
            return bTime - aTime;
          });
        ...
        callback({ success: false, conversations: [], error: error.message });
      });
    } catch (error) {
      console.error('Error starting conversations subscription:', error);
      callback({ success: false, conversations: [], error: error.message });
      return () => {};
    }
  },
  ...
  subscribeToMessages: (conversationId, callback) => {
    ...
    try {
      ...
      console.error('Error starting messages subscription:', error);
      callback({ success: false, messages: [], error: error.message });
      return () => {};
    }
  },
  ...
  sendChatMessage: async ({ senderId, recipientId, text }) => {
    try {
      ...
      const conversationId = firebaseHelpers.getConversationId(senderId, recipientId);
      const participants = [senderId, recipientId].sort();
      const conversationRef = doc(db, 'conversations', conversationId);
      const messageRef = doc(collection(db, 'conversations', conversationId, 'messages'));

      const [senderResult, recipientResult, conversationSnap] = await Promise.all([
        firebaseHelpers.getUserProfile(senderId).catch(() => ({ success: false })),
        firebaseHelpers.getUserProfile(recipientId).catch(() => ({ success: false })),
        getDoc(conversationRef)
      ]);

      const senderProfile = senderResult.success ? senderResult.profile : {};
      const recipientProfile = recipientResult.success ? recipientResult.profile : {};
      const senderName = senderProfile.displayName || senderProfile.username || senderProfile.email || 'A creator';
      const recipientName = recipientProfile.displayName || recipientProfile.username || recipientProfile.email || 'Creator';
      const senderAvatar = senderProfile.avatar || senderProfile.photoURL || null;
      const recipientAvatar = recipientProfile.avatar || recipientProfile.photoURL || null;
      ...
      batch.set(conversationRef, {
        ...conversationBase,
        createdAt: conversationSnap.exists() ? (conversationSnap.data().createdAt || serverTimestamp()) : serverTimestamp()
      }, { merge: true });
      ...
      firebaseHelpers.createNotification({
        recipientId,
        actorId: senderId,
  ...
  markConversationRead: async ({ conversationId, userId }) => {
    try {
      if (!conversationId || !userId) return { success: false, error: 'Missing conversation or user' };
  ...
  submitSupportRequest: async (supportData) => {
    try {
      const submitSupport = httpsCallable(functions, 'submitSupportRequest');
      const result = await submitSupport({
        type: supportData.type || 'support',
        name: supportData.name || '',
        email: supportData.email || '',
        subject: supportData.subject || supportData.reason || 'Support request',
        message: supportData.message || supportData.details || '',
        details: supportData.details || supportData.message || '',
        modelId: supportData.modelId || '',
        url: supportData.url || '',
        priority: supportData.priority || undefined
      });
  ...
  createReport: async (reportData) => {
    try {
      const functionResult = await firebaseHelpers.submitSupportRequest({
        ...reportData,
        source: reportData.source || 'report'
      });

      if (functionResult.success) {
        return { success: true, reportId: functionResult.ticketId };
      }
  ...
  getReports: async () => {
    try {
      ...
      if (error.code === 'permission-denied') {
        return { success: false, error: 'Reports are not readable with the current admin permissions.', reports: [] };
      }
  ...
  getModerationStats: async () => {
    try {
      ...
      if (error.code === 'permission-denied') {
        return {
          success: false,
          error: 'Moderation reports are not readable with the current admin permissions.',
