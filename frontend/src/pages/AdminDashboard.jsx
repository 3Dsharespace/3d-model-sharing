...
const AdminDashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const reportsResult = await firebaseHelpers.getReports()
  ...
  const supportTickets = useMemo(() => (
    reports
      .filter((report) => ['contact', 'support'].includes(report.source) || ['support', 'account', 'upload', 'download', 'bug', 'feedback'].includes(report.type))
      .sort((a, b) => (getDateValue(b.createdAt)?.getTime() || 0) - (getDateValue(a.createdAt)?.getTime() || 0))
  ), [reports])

  const moderationReports = useMemo(() => (
    reports
      .filter((report) => !supportTickets.some((ticket) => ticket.id === report.id))
      .sort((a, b) => (getDateValue(b.createdAt)?.getTime() || 0) - (getDateValue(a.createdAt)?.getTime() || 0))
  ...
  const updateReportStatus = async (reportId, action) => {
    ...
    const result = await firebaseHelpers.processReport(reportId, action, '', user.uid)
    if (!result.success) return

    setReports((current) => current.map((report) => (
      report.id === reportId
        ? { ...report, status: action === 'dismiss' ? 'dismissed' : 'resolved', action }
        : report
    )))
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-yellow-50 dark:from-gray-900 dark:via-red-900/20 dark:to-orange-900/20 flex items-center justify-center">
        ...
        >
          ...
          <Button onClick={() => window.location.href = '/explore'} className="px-6 py-3">
      ...
      >
        <div className="relative z-10">
          <div className="flex items-center justify-between">
              ...
              <p className="text-lg">
                Manage models, users, reports, and platform health from one place.
              </p>
                  </div>
  ...
  return (
    <div className="studio-admin min-h-screen">
      <PageMeta
        ...
        {activeTab === 'models' && (
              ...
              {activeTab === 'bulk-upload' && (
                ...
                >
                  ...
                  <Button onClick={() => window.location.href = '/admin/bulk-upload'}>
                    <UploadCloud className="w-4 h-4 mr-2" />
              ...
              {activeTab === 'reports' && (
                <motion.div key="reports" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Moderation reports</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{moderationReports.length}</div></div>
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Pending</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{moderationReports.filter(report => report.status === 'pending').length}</div></div>
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">High priority</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{moderationReports.filter(report => report.priority === 'high').length}</div></div>
                  </div>
                  <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Content and policy reports</h3>
                      <Button variant="outline" onClick={loadDashboardData}>Refresh</Button>
                    </div>
                    <div className="space-y-3 max-h-[70vh] overflow-y-auto">
                      {moderationReports.map((report) => (
                        <div key={report.id} className="border border-gray-100 dark:border-gray-700 rounded-xl p-4">
                          <div className="flex items-start justify-between gap-4">
                            <div>
                              <div className="flex items-center gap-2 mb-2">
                                <span className="text-sm font-semibold text-gray-900 dark:text-white">{(report.type || 'report').replace(/_/g, ' ')}</span>
                                <span className="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{report.status || 'pending'}</span>
                                <span className="text-xs px-2 py-1 rounded bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-300">{report.priority || 'medium'}</span>
                              </div>
                              <p className="text-sm text-gray-700 dark:text-gray-300 mb-2">{report.details}</p>
                              <div className="text-xs text-gray-500 dark:text-gray-400 space-x-3">
                                <span>{report.email}</span>
                                <span>{formatDateTime(report.createdAt)}</span>
                                {report.modelId && <span>Model {report.modelId}</span>}
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                              {report.url && <Button size="sm" variant="outline" onClick={() => window.open(report.url, '_blank')}>Open URL</Button>}
                              {report.status === 'pending' && <Button size="sm" variant="outline" onClick={() => updateReportStatus(report.id, 'resolve')}>Resolve</Button>}
                              {report.status === 'pending' && <Button size="sm" variant="outline" onClick={() => updateReportStatus(report.id, 'dismiss')}>Dismiss</Button>}
                            </div>
                          </div>
                        </div>
                      ))}
                      {!moderationReports.length && <div className="text-sm text-gray-500 dark:text-gray-400">No moderation reports yet.</div>}
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {activeTab === 'support' && (
                <motion.div key="support" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Support tickets</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{supportTickets.length}</div></div>
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Pending</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{supportTickets.filter(ticket => ticket.status === 'pending').length}</div></div>
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Upload issues</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{supportTickets.filter(ticket => ticket.type === 'upload').length}</div></div>
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700"><div className="text-sm text-gray-500 dark:text-gray-400">Bugs</div><div className="text-2xl font-bold text-gray-900 dark:text-white">{supportTickets.filter(ticket => ticket.type === 'bug').length}</div></div>
                  </div>
                  <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Support inbox</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Contact page messages appear here and also notify admins in-app.</p>
                      </div>
                      <Button variant="outline" onClick={loadDashboardData}>Refresh</Button>
                    </div>
                    <div className="space-y-3 max-h-[70vh] overflow-y-auto">
                      {supportTickets.map((ticket) => (
                        <div key={ticket.id} className="border border-gray-100 dark:border-gray-700 rounded-xl p-4">
                          <div className="flex items-start justify-between gap-4">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-2">
                                <span className="text-sm font-semibold text-gray-900 dark:text-white">{ticket.subject || 'Support request'}</span>
                                <span className="text-xs px-2 py-1 rounded bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-300">{ticket.type || 'support'}</span>
                                <span className="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{ticket.status || 'pending'}</span>
                              </div>
                              <p className="text-sm text-gray-700 dark:text-gray-300 mb-3 whitespace-pre-wrap">{ticket.details || ticket.message}</p>
                              <div className="text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-x-3 gap-y-1">
                                <span>{ticket.name || 'No name'}</span>
                                <span>{ticket.email}</span>
                                <span>{formatDateTime(ticket.createdAt)}</span>
                              </div>
                            </div>
                            <div className="flex shrink-0 items-center gap-2">
                              {ticket.url && <Button size="sm" variant="outline" onClick={() => window.open(ticket.url, '_blank')}>Open URL</Button>}
                              {ticket.status === 'pending' && <Button size="sm" variant="outline" onClick={() => updateReportStatus(ticket.id, 'resolve')}>Resolve</Button>}
                              {ticket.status === 'pending' && <Button size="sm" variant="outline" onClick={() => updateReportStatus(ticket.id, 'dismiss')}>Dismiss</Button>}
                            </div>
                          </div>
                        </div>
                      ))}
                      {!supportTickets.length && <div className="text-sm text-gray-500 dark:text-gray-400">No support tickets yet.</div>}
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {!['dashboard', 'models', 'all-users', 'bulk-upload', 'health', 'analytics', 'logs', 'backups', 'access-control', 'audit-logs', 'sessions', 'overview', 'downloads', 'engagement', 'maintenance', 'admins', 'creators', 'reports', 'support'].includes(activeTab) && (
                <motion.div 
                  key="default"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                >
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-6">
                    This admin tool has not been connected yet.
                  </p>
