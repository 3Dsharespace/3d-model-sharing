...
const UPLOAD_DRAFT_VERSION = 2

const normalizeAiEndpoint = (value) => String(value || '').trim().replace(/\/+$/, '')

const getUploadDraftKey = (userId) => `upload_draft_${userId}`
...
const fillerNameTokens = new Set([
  '3d',
  'model',
  'asset',
  'render',
...
const categoryRules = [
  ...
  { category: 'Shoes', tokens: ['shoe', 'shoes', 'sneaker', 'boot', 'sandal', 'slides', 'loafer', 'loafers'] },
...
const tokenizeName = (...values) => values
  ...
  .filter((token) => token && !fillerNameTokens.has(token) && !/^\d+$/.test(token))
...
const inferCategory = (tokens) => {
  ...
  return modelCategoryNames.includes(category) ? category : 'Other'
}
...
const getAutoDetails = ({ title, file, previewImages = [] }) => {
  const rawTitle = toTitleCase(title || getNameWithoutExtension(file?.name) || 'Free 3D Model')
  const sourceNames = [
    rawTitle,
    file?.name,
    ...previewImages.map((image) => image.file?.name || image.name)
  ]
  ...
  const renderText = previewImages.length
    ? `The upload includes ${previewImages.length} preview render${previewImages.length !== 1 ? 's' : ''} to help users inspect the asset before download.`
    : 'Add preview renders to help users inspect the asset before download.'
  const materialText = materials.length
    ? `Detected material or style words include ${formatWordList(materials)}.`
    : 'The asset is prepared for general 3D visualization, product scenes, and creative layout work.'
  const colorText = colors.length
    ? `Detected color details include ${formatWordList(colors)}.`
    : ''
  ...
  const formatText = format
    ? `The main file is provided in ${format} format for compatible 3D software.`
    : 'The main model file is ready for compatible 3D software.'

  return {
    title: rawTitle.slice(0, 100),
    category,
    tags: tags.join(', '),
    description: `${rawTitle} is a free 3D model in the ${category} category${readableTags ? `, with details such as ${readableTags}` : ''}. ${formatText} ${renderText} ${materialText} ${colorText} ${useCaseText} Use this asset for Blender, 3ds Max, Maya, Cinema 4D, game engines, AR/VR projects, product presentation, and 3D visualization.`.replace(/\s+/g, ' ').trim()
  }
}
...
const Upload = () => {
  ...
  useEffect(() => {
    ...
    const draftKey = getUploadDraftKey(user.uid)
    const savedDraft = localStorage.getItem(draftKey)
    if (!savedDraft) return

    try {
      const draft = JSON.parse(savedDraft)
      if (draft.draftVersion !== UPLOAD_DRAFT_VERSION) {
  ...
  function saveDraft(showMessage = true) {
    ...
    const draft = {
      ...formData,
      draftVersion: UPLOAD_DRAFT_VERSION,
      modelFileName: selectedFile?.name || '',
      timestamp: new Date().toISOString(),
      userId: user.uid
    }

    localStorage.setItem(getUploadDraftKey(user.uid), JSON.stringify(draft))
  ...
  const clearUploadForm = (message = 'Upload form cleared.') => {
    ...
    if (user) localStorage.removeItem(getUploadDraftKey(user.uid))
  }
  ...
  const removeFile = () => {
    setSelectedFile(null)
    setSuccess('Model file removed. Add a new file or clear the form to start fresh.')
    if (fileInputRef.current) fileInputRef.current.value = ''
  }
  ...
  const generateWithLocalAi = async () => {
    ...
    const applyFastSmartFill = () => {
      ...
      setFormData((prev) => ({
        ...prev,
        title: prev.title.trim() || autoDetails.title,
        category: prev.category || autoDetails.category,
        description: prev.description.trim() || autoDetails.description,
        tags: prev.tags.trim() || autoDetails.tags
      }))
    }
    ...
    if (!aiServiceUrl) {
      applyFastSmartFill()
      setSuccess('Fast smart fill added details. Review them before uploading.')
      return
    }
    ...
    try {
      ...
      console.warn('Auto-fill unavailable.', err)
      applyFastSmartFill()
      setSuccess('Fast smart fill added details. The AI helper is offline, but upload can continue.')
    } finally {
      ...
      <div className="studio-container">
        <div className="mb-8 flex flex-col gap-4 border-b border-[#242424] pb-6 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-[#737373]">
            ...
            <Button type="button" variant="outline" onClick={() => clearUploadForm('Fresh upload form ready.')} className="border-[#242424] bg-[#050505] text-[#d4d4d4] hover:bg-[#101010]">
              New Upload
            </Button>
