...
const writeInBatches = async (items, handler, batchSize = 24) => {
...
try {
  ...
  await writeInBatches(models, async (model) => {
    ...
    await writeInBatches(images, async (_image, index) => {
  ...
  await writeInBatches(galleryPages, async (page) => {
