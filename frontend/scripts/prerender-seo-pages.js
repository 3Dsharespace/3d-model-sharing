...
const jsonLdForPage = (page) => [
  ...
  },
  {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: getSeoFaqs(page).map((faq) => ({
      '@type': 'Question',
      name: faq.question,
      acceptedAnswer: {
        '@type': 'Answer',
...
const renderStaticPage = (page) => `
  <main class="discovery-prerender">
    ...
    <section class="discovery-faq">
      <h2>Questions about ${escapeHtml(page.name)} assets</h2>
      <div>
        ${getSeoFaqs(page).map((faq) => `
          <article>
...
const pageHead = (page) => `
    ...
    <style>
      ...
      .discovery-faq{max-width:1120px;margin:42px auto 0;border-top:1px solid #262626;padding-top:28px}
      .discovery-faq>div{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
      .discovery-faq article{border:1px solid #262626;background:#0d0d0d;padding:20px}
      .discovery-faq h2{margin:0 0 18px;font-size:24px}
      .discovery-faq h3{margin:0 0 10px;font-size:16px}
      .discovery-faq p{margin:0;color:#d4d4d4;line-height:1.65}
      @media (max-width:800px){.discovery-grid{grid-template-columns:1fr}.discovery-prerender{padding:44px 18px}}
      @media (max-width:800px){.discovery-faq>div{grid-template-columns:1fr}}
    </style>
`
